...
def test_creation_and_extraction_of_tensor_proto():
    ...
    recreated_array = utils.create_array_from_proto(tensor_proto=my_proto)
    assert np.array_equal(recreated_array, np.array([e.encode() for e in array]).reshape(2, 2))

    array = np.array(["Test1", "test2", "random stuffs", "This is a string"], dtype=object).reshape((2, 2))
    my_proto = utils.create_tensor_proto(array)
    recreated_array = utils.create_array_from_proto(tensor_proto=my_proto)
    assert np.array_equal(recreated_array, np.array([e.encode() for e in array.flatten()]).reshape(2, 2))
